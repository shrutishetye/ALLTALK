...
#End Region
    ...
    Public _Ip As IPAddress
    Public _Rep As UShort
    ...
    '        Dim remoteEP As New IPEndPoint(_Ip, Convert.ToInt32(port))
    ...
    '        'End If
    '        Handle.BeginConnect(remoteEP, New AsyncCallback(AddressOf ConnectCallback), Handle)
    '        connectDone.WaitOne()
    ...
    '        Dim clientHandler As Socket = DirectCast(ar.AsyncState, Socket)

    '        ' Complete the connection.
    '        clientHandler.EndConnect(ar)

    '        MessageBox.Show("Socket connected")

    '        ' Signal that the connection has been made.
    '        connectDone.[Set]()
    '        Items(0).RemoteEndPoint = New IPEndPoint(_Ip, _Rep)
    '        If Not Handle.ConnectAsync(Items(0)) Then
    '            Items(0).SocketError = SocketError.Success
    '            Process(Nothing, Items(0))
    '        End If
    ...
    '        MessageBox.Show("Socket connected2")
    '        O.Post(Sub(x) OnExceptionThrown(DirectCast(x, Exception)), e)
    '        Disconnect()
    '    End Try
    'End Sub
    ...
    Private Sub Process(s As Object, e As SocketAsyncEventArgs)
        'MessageBox.Show("Socket connectedPPPPPPP75765")

        Try
            If e.SocketError = SocketError.Success Then
                Select Case e.LastOperation
                    Case SocketAsyncOperation.Connect
                        _EndPoint = New IPEndPoint(_Ip, _Rep)
                        _Connected = True
